function [PrI_sal, PrI_bg, PrO_sal, PrO_bg] = likelihoodprobSPSpeedUpNoWeight(input_imlab, imglbp, ind, out_ind, pixelListSlic, hullSP, Wgeo)
%% Ackonwledegment
% this code is modified from [1]
% [1] Yulin Xie, Huchuan Lu, M.-H. Y. (2013). Bayesian Saliency via Low and Mid Level Cues. In TIP (Vol. 22, pp. 1757–1761).
...
smoothFactor=[5,6,6,6,6,6,6]; % Smoothing factor
...
% %% hist
% input_imlab_Ind_In = input_imlab * 0 ;
% input_imlab_Ind_Out = input_imlab * 0 ;
% for i = 1: 3
%     cur_im = input_imlab(:,:,i);
%     input_imlab_Ind_In(:,:, i) = round( (cur_im - minValI(i)) ./ ( maxValI(i) - minValI(i) ) * (numBin(i) -  1) + 1);
%     input_imlab_Ind_Out(:,:, i) = round( (cur_im - minValO(i)) ./ ( maxValO(i) - minValO(i) ) * (numBin(i) -  1) + 1);
% end
% 
% i = 4;
% 
% imglbp_Ind_In  = round( (imglbp - minValI(i)) ./ ( maxValI(i) - minValI(i) ) * (numBin(i) -  1) + 1);
% imglbp_Ind_Out = round( (imglbp - minValO(i)) ./ ( maxValO(i) - minValO(i) ) * (numBin(i) -  1) + 1);
...
for k = 1: spNum
    if ismember(k, hullSP)
        ...
        for i = 1: 3
            ...
            %             tic
            %             innerHist=zeros(numBin(i),1);
            %             binIndTmp = tabulate(dataMat);
            %             innerHist(1:max(dataMat)) = binIndTmp(:, 2);
            %             toc
            
            
            
            %             [innerHist,binInd] = ComputeHistogram(cur_im(pixelListSlic{k}),numBin(i),minValI(i),maxValI(i));
...
%         innerHist = filterDistribution_(smoothingKernel{i},innerHist',numBin(i));
        ...
        PrI_Hist = PrI_Hist + PrI_salSP  * 1 ;%*  mean(LbpSim(k, hullSP));
        Sum_W_Sal = Sum_W_Sal +  1;% *  mean(LbpSim(k, hullSP));
        ...
        for i = 1: 3
            ...
            %             [outerHist,~] = ComputeHistogram(dataMat,numBin(i),minValO(i),maxValO(i));
            %
%             smoothingKernel{i}=getSmoothKernel_(smoothFactor(i));
%             outerHist=filterDistribution_(smoothingKernel{i},outerHist',numBin(i));
        ...
        %         dataMat = imglbp(pixelListSlic{k});
        %         [outerHist, ~] = ComputeHistogram(dataMat, numBin(i),minValO(i),maxValO(i));
%         smoothingKernel{i} = getSmoothKernel_(smoothFactor(i));
%         outerHist = filterDistribution_(smoothingKernel{i},outerHist',numBin(i));
        PrO_bgSP(delta: delta + length(outerHist) - 1) = outerHist;
        
        PrO_Hist = PrO_Hist + PrO_bgSP  * 1;% *  mean(LbpSim(k, hullSP));
        Sum_W_Bg = Sum_W_Bg +  1;% *  mean(LbpSim(k, hullSP));
    end
end
...
for i = 1: 3
    %     cur_im = input_imlab(:,:,i);
    %     dataMat = cur_im(ind);
    %     inInd = max( min(ceil(numBin(i) * (double(dataMat - minValI(i)) / (maxValI(i) - minValI(i)))), numBin(i)), 1);
    %     dataMat = cur_im(out_ind);
    %     outInd = max( min(ceil(numBin(i) * (double(dataMat -minValO(i)) / (maxValO(i) - minValO(i)))), numBin(i)), 1);
