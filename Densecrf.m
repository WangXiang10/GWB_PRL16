...
% 		
	...
	methods
		...
		function settings = gather_settings(self)
            ...
            settings.bilateral_lbp_stddev = self.bilateral_lbp_stddev;
            ...
            settings.Lbpbins = int32(self.Lbpbins);
		end
		
		function self =  Densecrf(ColorHistDist, LbpHistDist, unary, meanPos)
...
% 			end
            ...
            self.spNum = uint32(size(meanPos, 1));
% 			assert(numel(self.image_size) == 3);
			
			self.ColorHistDist = single(ColorHistDist);
            self.LbpHistDist = single(LbpHistDist);
			self.unary = single(unary);
            self.meanPos = single(meanPos);
            
			self.get_energy = false;
			self.segmentation = ones(size(meanPos,1), 1);
			self.get_energy = true;

		end

		% Compile if need be
		function compile(~, file_name,ifg)
            if nargin < 3
                ifg = 0;
            end
			...
			compile(cpp_file, out_file, sources, extra_arguments,ifg)
		end
		...
		function segmentation = mean_field_explicit(self)
			...
			t = tic;
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
			segmentation =  densecrf_mex(self.im_stacked, self.lbp, self.unary_stacked, self.idxImg, self.comSim, self.image_size, self.spNum, settings);
			%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%             settings.solver = 'mean_field_explicit';
			...
			t = tic;
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
			[segmentation, energy, bound] =  densecrf_mex(self.ColorHistDist_stacked, self.LbpHistDist_stacked, self.unary_stacked, self.meanPos(:,1), self.meanPos(:,2), settings);
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
           self.optimization_time = toc(t);
...
% 		end
        ...
        function save(self, filename, pixelList, h, w)	
            salMap = zeros(h, w);
            seg = self.segmentation;
%             save([filename, '.mat'], 'seg')
            seg = seg / max(max(seg));
            seg(seg == 0.5) = 0;
            spnum = size(pixelList, 1);
            for sp = 1: spnum
               salMap(pixelList{sp}) =  seg(sp);
            end
            imwrite(salMap, [filename]);
        ...
        function set.ColorHistDist(self, ColorHistDist)
			self.ColorHistDist = ColorHistDist;
			self.ColorHistDist_stacked = reshape(self.ColorHistDist',numel(self.ColorHistDist), 1);
        end
        
        function set.LbpHistDist(self, LbpHistDist)
			self.LbpHistDist = LbpHistDist;
            self.LbpHistDist_stacked = reshape(self.LbpHistDist',numel(self.LbpHistDist), 1);
        end
        function set.unary(self, unary)
            self.unary = unary;
            self.unary_stacked = reshape(self.unary',numel(self.unary), 1);
        end
...
%         end
%         function set.lbp(self, lbp)
% 			self.lbp = lbp;
% 			self.lbp_stacked = single(Densecrf.color_stack(lbp));
%         end
%         
%         function set.idxImg(self, idxImg)
% 			self.idxImg = idxImg;
% 			self.idxImg_stacked = single(Densecrf.color_stack(idxImg));
%         end
%         
%         function set.comSim(self, comSim)
% 			self.comSim = comSim;
% 			self.comSim_stacked = single(Densecrf.color_stack(comSim));
%         end
